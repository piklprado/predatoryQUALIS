...
do Brasil publicam seus artigos. A base é usada na avaliação da produção dos programas, feita pela CAPES.
...
totalizou  `r nrow(beals)+ nrow(sc.bealsp)` títulos, sendo  `r nrow(beals)` *standalone*
...
`r round(100*nrow(pred.titles)/(nrow(beals)+ nrow(sc.bealsp)),1)`% da lista que compilamos.
...
nos estratos A e B por pelo menos uma área. Periódicos potenciailmente predatórios estão presentes em todos os estratos, 
...
* Uma proporção pequena do total de periódicos é potencialmente predadora. No entanto, a lista de potenciais predadores que compilamos não está completa. Ainda assim, parece pouco provável que a proporção real
